...
def build_tool_registry(controller: DroneController, safety: SafetyChecker) -> ToolRegistry:
    ...
    registry.register(ToolSpec("request_route_replan", "Return safe route/viewpoint alternatives for a visible constraint.", required={"reason"}, handler=inspection.request_route_replan))
    ...
    registry.register(ToolSpec("move_to_asset", "Move to a safe standoff viewpoint for a visible asset.", required={"asset_id", "standoff_bucket"}, optional={"speed_mps"}, handler=flight.move_to_asset))
    ...
    registry.register(ToolSpec("set_camera_source", "Set active camera source to rgb, thermal, or rgb_thermal.", required={"source"}, handler=camera.set_camera_source))
    registry.register(ToolSpec("point_camera_at", "Point the camera/gimbal at a visible asset ROI.", required={"asset_id"}, handler=camera.point_camera_at))
